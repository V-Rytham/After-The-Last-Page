...
export const updateUserBookAccess = async (req, res) => {
  try {
    const { books } = req.body;

    if (!Array.isArray(books) || books.length === 0) {
      return res.status(400).json({ message: 'At least one book access update is required.' });
    }
    ...
    books.forEach((entry) => {
      ...
      const existingIndex = user.booksRead.findIndex((item) => item.bookId === entry.bookId);
      const existing = existingIndex >= 0 ? user.booksRead[existingIndex] : null;

      const nextEntry = {
        bookId: entry.bookId,
        hoursSimulated: entry.hoursSimulated ?? existing?.hoursSimulated ?? 0,
      ...
      if (existingIndex >= 0) {
        user.booksRead[existingIndex] = nextEntry;
      } else {
        user.booksRead.push(nextEntry);
      }
    });
